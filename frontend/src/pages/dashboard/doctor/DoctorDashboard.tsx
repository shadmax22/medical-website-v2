...
export function DoctorDashboard() {
  const [doctorData, setDoctorData] = useState<DoctorDashboardData | null>(
    null
  );
  ...
  if (loading) return <Loader message="Loading doctor dashboard..." />;
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <Card className="xl:col-span-2 border border-blue-gray-100 shadow-sm">
          ...
          <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
            <table className="w-full min-w-[640px] table-auto">
              ...
              <tbody>
                {doctor_data.assigned_patients.map(
                  ({ name, age, issue, img, last_visit }, index) => (
                    <tr key={index}>
        ...
        <Card className="border border-blue-gray-100 shadow-sm">
          ...
          <CardBody className="pt-0">
            {doctor_data.active_patients.map(
              ({ name, issue, time, img }, index) => (
                <div
                  key={index}
